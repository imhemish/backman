...
bc[python]. sudo ./install

p. Prepending the install script with "sudo" or executing in super user environment is required.

p. If you are using **Arch Linux**, there is an **AUR** package, called **"backman"**. You can install by cloning or using an AUR helper.

p. If you are using Debian and are familiar with Makedeb Package Repository, there is a package there, called "backman". You can install by cloning or using helper.
...
bc.. usage: backman [-h] [--set] [--return-path] [--wal] [--change-mode mode] [--set-fix-bg /path/to/file]
...
bc[python]. backman -m fixed

p. To change to random, you can execute:

bc[python]. backman -m random
...
bc[python]. backman -i /home/user/Pictures/1.jpg
...
bc[python]. backman --add-dir /usr/share/backgrounds --rm-dir /home/user/Pictures
...
bc[python]. backman -r 2>/dev/null
...
bc[python]. backman -s 2>/dev/null 
